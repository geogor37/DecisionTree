...
//		else if (model.equals("multiPerceptron")) return new MultiPerceptron(rand);
//		else if (model.equals("neuralnet")) return new NeuralNet(rand);
	...
	public void run(String[] args) throws Exception {
		...
		if (evalMethod.equals("training")) {
			...
			double accuracy = learner.measureAccuracy(features, labels, confusion, false);
			...
			double elapsedTime = System.currentTimeMillis() - startTime;
			System.out.println("Time to train (in seconds): " + elapsedTime / 1000.0);
			double trainAccuracy = learner.measureAccuracy(features, labels, null, false);
			...
			double elapsedTime = System.currentTimeMillis() - startTime;
			System.out.println("Time to train (in seconds): " + elapsedTime / 1000.0);
			double trainAccuracy = learner.measureAccuracy(trainFeatures, trainLabels, null, false);
			System.out.println("Training set accuracy: " + trainAccuracy);
			Matrix confusion = new Matrix();
			double testAccuracy = learner.measureAccuracy(testFeatures, testLabels, confusion, true);
			System.out.println("Test set accuracy: " + testAccuracy);
			...
			for (int j = 0; j < reps; j++) {
				...
				for (int i = 0; i < folds; i++) {
					...
					double accuracy = learner.measureAccuracy(testFeatures, testLabels, null, false);
