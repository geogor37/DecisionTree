...
public abstract class SupervisedLearner {
	...
	public abstract void predict(double[] features, double[] labels, double[] target, boolean isTest) throws Exception;
	...
	public double measureAccuracy(Matrix features, Matrix labels, Matrix confusion, boolean isTest) throws Exception
	{
		...
		else
		{
			...
			for(int i = 0; i < features.rows(); i++)
			{
				...
				double[] targetArray = {targ};
				if(targ >= labelValues)
					throw new Exception("The label is out of range");
				predict(feat, prediction, targetArray, isTest);
